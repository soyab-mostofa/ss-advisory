  ...
  useEffect(() => {
    const wrapText = () => {
      ...
      words.forEach((word, index) => {
        ...
        const spaceWidth =
          index > 0 ? (tempSpan.offsetWidth / word.length) * 1 : 0; // Approximate space width

        if (
          currentLine.length === 0 ||
          currentLineWidth + spaceWidth + wordWidth <= containerWidth
        ) {
          currentLine.push(word);
          currentLineWidth +=
            (currentLine.length > 1 ? spaceWidth : 0) + wordWidth;
        } else {
  ...
  useEffect(() => {
    ...
    if (startHighlightSpans.length > 0) {
      masterTl.to(
        ...
        ">+=0.3"
      ); // Start 0.3s after text reveal completes
    }
    ...
    if (endHighlightSpans.length > 0) {
      masterTl.to(
        ...
        ">+=0.2"
      ); // Start 0.2s after starting highlight completes
    }
